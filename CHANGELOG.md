...
- **`@litopys/notion-sync` package** (`packages/notion-sync/`). Reads recently edited Notion pages via the Notion REST API, extracts knowledge candidates through the configured LLM adapter, and writes results to quarantine — same pipeline as the Claude Code session extractor. Designed to run as a systemd timer every 6 hours. Reads `NOTION_TOKEN` from env or `~/.litopys/.env`. State (last-sync timestamp) is persisted to `notion-sync.json` next to the graph directory so re-runs only process pages edited since the previous tick.
