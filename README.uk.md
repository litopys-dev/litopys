...
**[v0.2.0](https://github.com/litopys-dev/litopys/releases/tag/v0.2.0) вийшла** — бітемпоральна модель (`occurred_at` / `since` / `until` на кожному вузлі, запити `as_of`), команда обслуговування `litopys evolve` (архівація надгробкових вузлів, авто-злиття пропозицій із високою впевненістю) і новий пакет `@litopys/bench` — гарнес бенчмарків із детермінованим мок-екстрактором — див. [CHANGELOG](./CHANGELOG.md). Готові двійкові файли для Linux / macOS / Windows (x64 + arm64) із SHA-256 контрольними сумами, що перевіряються `install.sh`. Тестовий набір — **613 / 613** успішних. Публічні інтерфейси (MCP tools, CLI, JSON-експорт `schemaVersion: 1`, розташування markdown на диску) залишаються зворотньо сумісними; подальші несумісні зміни вийдуть у версії `0.3.x`.
...
Екстрактор тепер підтримує будь-який OpenAI-сумісний сервер — vLLM, LM Studio, LocalAI, Ollama `/v1` проксі — через дві змінні середовища:
