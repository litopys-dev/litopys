...
**[v0.1.4](https://github.com/litopys-dev/litopys/releases/tag/v0.1.4) вийшла** — візуальне оновлення графу, автоматична генерація токена для вебпанелі, інтеграція Notion sync та підтримка OpenAI-сумісних ендпоінтів в екстракторі — див. [CHANGELOG](./CHANGELOG.md). Готові двійкові файли для Linux / macOS / Windows (x64 + arm64) із SHA-256 контрольними сумами, що перевіряються `install.sh`. Публічні інтерфейси (MCP tools, CLI, JSON-експорт `schemaVersion: 1`, розташування markdown на диску) заморожені; зворотньо несумісні зміни вийдуть у версії `0.2.x`.
