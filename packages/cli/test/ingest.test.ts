...
// ---------------------------------------------------------------------------
// Per-file mock response — candidate with non-zero tokens (several tests assert on counts)
// ---------------------------------------------------------------------------
...
const anthropicCreate = mock(async () => ({
  content: [
    {
      type: "text",
      text: JSON.stringify({
...
beforeAll(() => {
  setAnthropicCreate(anthropicCreate);
  setOpenAICreate(openaiCreate);
});
