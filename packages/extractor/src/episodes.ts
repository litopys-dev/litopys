 ...
 * Placeholders: {minToolOps}, {lang}, {transcript}
 */
...
For each episode output: goal (short, in {lang}), steps (3-10 generalized imperative steps, {lang}), toolOps (count of TOOL: lines belonging to the episode), errorRecovery (true if the solution was found after 2+ failed attempts — look for "→ error" followed by retries), project (best guess from paths/names, else null), tags (2-5 lowercase english).
...
export async function extractEpisodes(
  ...
  opts: { minToolOps: number; lang?: string },
): Promise<Episode[]> {
